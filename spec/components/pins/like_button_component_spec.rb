...
RSpec.describe Pins::LikeButtonComponent, type: :component do
  ...
  context 'with current_user' do
    context 'if it is unliked' do
      ...
      it 'path' do
        ...
        expect(rendered_component).to have_css ".pin-likes-count .like_btn[href~='#{href}']"
      end
    end
    ...
    context 'if it is liked' do
      ...
      it 'path' do
        ...
        expect(rendered_component).to have_css ".pin-likes-count .like_btn.liked[href~='#{href}']"
      end
    end
  end
  ...
  context 'without current_user' do
    before do
      render_inline(described_class.new(pin: pin, current_user: nil))
    end

    it 'path' do
      expect(rendered_component).to have_css ".pin-likes-count .like_btn.disabled[href~='#']"
    end
  end
end
