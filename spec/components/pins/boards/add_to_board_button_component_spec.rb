...
RSpec.describe Pins::Boards::AddToBoardButtonComponent, type: :component do
  ...
  describe 'dropdown div' do
    it 'id' do
      expect(rendered_component).to have_css "div[id='add-pin-to-board-dropdown_pin_#{pin.id}']"
    end

    it 'controller' do
      expect(rendered_component).to have_css 'div[data-controller="dropdown"]'
    end

    it 'classes' do
      expect(rendered_component).to have_css 'div[class="dropdown dropleft add-pin-to-board-dropdown"]'
    end

    it 'controller id value' do
      expect(rendered_component).to have_css "div[data-dropdown-id-value='add-pin-to-board-dropdown_pin_#{pin.id}']"
    end
  end

  describe 'dropdown-menu button' do
    it 'id' do
      expect(rendered_component).to have_css 'a[id="add-pin-to-board-menu"]'
    end
    ...
    context 'if added any pin to board' do
      ...
      it 'class' do
        expect(rendered_component).to have_css 'a[class="add_to_board_btn added"]'
      end
    end

    context 'if there is no added pin to board' do
      it 'class' do
        expect(rendered_component).to have_css 'a[class="add_to_board_btn"]'
      end
    end
  end
  ...
  describe 'dropdown-menu div' do
    it 'class' do
      expect(rendered_component).to have_css 'div[class="dropdown-menu add-pin-to-board"]'
    end
  ...
  describe 'header' do
    it 'dropdown header text' do
      expect(rendered_component).to have_css '.header span', text: I18n.t('boards.add_to_board')
    end
  end
  ...
  context 'when there is board' do
    ...
    describe 'header' do
      it 'renders search form' do
        expect(rendered_component).to have_css '.header .search-form'
      end
    end
    ...
    describe 'body' do
      ...
      context 'turbo frame' do
        ...
        it 'loading' do
          expect(rendered_component).to have_css 'turbo-frame[loading="lazy"]'
        end
